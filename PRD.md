...
1. ~~**회원사(기관) 단위 멀티테넌시 관리 (최상위 핵심 기능)**~~
    - ~~**회원사(기관)별 완전한 데이터·정책·사용자 격리 구조 제공**~~
        - ~~DB 논리적 분리(스키마/테넌트 ID), 캐시·로그·파일스토리지 등 **모든 계층에서 격리 보장**~~
        - ~~회원사별로 **정책·룰·탐지 기준·임계치·스냅샷**을 **독립 관리**, 상호 접근 금지~~
        - ~~회원사별 **사용자/역할(Role)**, **조직/결재선**, **SMTP·보고·연계 설정** 등 **개별 운영 가능**~~
    - ~~**테넌트 전용 관리자(Admin) 기능**~~
        - ~~각 회원사 내 전용 **시스템 관리자**, **준법 책임자**, **조직 승인자** 지정 가능~~
        - ~~본사(운영사) 또는 중앙관리자는 **테넌트 등록·삭제·버전배포 통제권한** 보유~~
        - ~~테넌트별 **버전 스냅샷 배포 이력** 및 **감사로그 분리 저장**~~
    - ~~**보안/컴플라이언스**~~
        - ~~회원사 간 네트워크/데이터 완전 분리 (VPC/Subnet 또는 Schema 기반 멀티테넌시)~~
        - ~~규제기관 요청 시 회원사 단위로 **독립 보고 가능**~~
    - ~~**확장성/운영성**~~
        - ~~신규 회원사 Onboarding 프로세스 (템플릿 기반 초기 정책 세팅)~~
        - ~~회원사별 **룰·기준·스냅샷 Export/Import** 지원(권한 승인 하에만)~~
2. **정책·기준 관리 모듈(스냅샷)**
    - **KYC 위험평가 기준 관리**
